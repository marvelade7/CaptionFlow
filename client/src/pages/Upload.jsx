...
function StatusBadge({ status }) {
    const map = {
        ...
        processing: {
            label: "Processing…",
            cls: "bg-purple-50 text-[#7c3aed] border-purple-100",
        },
        completed: {
            label: "Completed",
    ...
    return (
        <span
            className={`rounded-full border px-3 py-0.5 text-xs font-semibold ${cls}`}
        >
            {label}
        </span>
    );
...
export default function Upload() {
    ...
    const validate = useCallback((f) => {
        ...
        if (!ACCEPTED.includes(ext))
            return `Unsupported format. Allowed: ${ACCEPTED.join(", ")}`;
        if (f.size > MAX_BYTES)
    ...
    useEffect(() => {
        if (location.state?.pendingFile) {
            ...
            if (!err) {
                setFile(pending);
                toast.success(`Loaded "${pending.name}" ready for upload!`, {
                    id: "preload-file",
                });
            } else {
                setError(err);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            if (pollStartTime && Date.now() - pollStartTime > POLL_TIMEOUT_MS) {
                ...
                setError(
                    "Transcription is taking too long — this may be a connection issue. Please try again.",
                );
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            ...
            const raw =
                Math.log1p((elapsed / ESTIMATED_DURATION_MS) * (Math.E - 1)) *
                99;
    ...
    const downloadAIPdf = () => {
        ...
        doc.save(
            `${job?.originalFileName?.split(".")[0] || "transcript"}-AI-Summary.pdf`,
        );
    };
    ...
    const copyAIText = (text, id) => {
        ...
        setTimeout(
            () => setAiCopiedStates((prev) => ({ ...prev, [id]: false })),
            2000,
        );
    };
    ...
    const copyAllExcerpts = () => {
        ...
        const text = aiResult.excerpts.items
            .map((item, i) => `${i + 1}. ${item}`)
            .join("\n\n");
            ...
            >
                ...
                {!jobId && (
                    <div
                        ...
                        className={`relative flex min-h-55 w-full flex-col items-center justify-center rounded-2xl border-2 border-dashed bg-white transition-all sm:min-h-75
                        ...
                        {!file ? (
                            ...
                            <div className="flex w-full flex-col items-center gap-4 px-8">
                                ...
                                <div className="text-center">
                                    <p className="text-sm font-bold text-[#0f0b1f]">
                                        {file.name}
                                    </p>
                                    <p className="mt-0.5 text-xs text-[#6b6680]">
                    ...
                    <div className="w-full overflow-hidden rounded-2xl border border-[#ecebf3] bg-white shadow-[0_1px_2px_rgba(15,11,31,0.04),0_8px_24px_rgba(15,11,31,0.04)]">
                        ...
                        <div className="flex flex-col gap-3 border-b border-[#ecebf3] px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                            <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                                <p className="max-w-40 truncate text-sm font-semibold text-[#0f0b1f] sm:max-w-65">
                            ...
                            <div className="flex flex-col items-center gap-4 px-4 py-8 sm:px-8 sm:py-10">
                                ...
                                <div className="w-full max-w-sm">
                                    ...
                                    <div className="h-2.5 w-full overflow-hidden rounded-full bg-[#ede9fe]">
                                        <div
                                            className="h-full rounded-full bg-linear-to-r from-[#7c3aed] to-[#a78bfa] transition-all duration-500 ease-out"
                                            style={{
                                                width: `${transcriptionPct}%`,
                                            }}
                                        />
                                    </div>
                                </div>
                                ...
                                <p className="text-xs text-[#a8a3bd]">
                                    Larger files take longer — you can leave
                                    this tab open.
                                </p>
                            </div>
                        )}
